...
come after the `Started` event if they were generated by the same entity (fish). This is why, when processing those events,
...
by the differential dataflow, which is not `Serde` as most of the Rust code does, but `Abomonation`. This is a very efficient binary wire
...
... TODO ... here point out the lot of copying but also point out points

There are two additional points to be made about the code above. First ( `// 1` ) our event design is not ideal, in each event
...
of the string gets duplicated. However, as `.clone()` in Rust is about ownership semantics
and not actual copying of the information, this problem can be easily side-stepped.

The initial instinct would be then to use the `std::borrow::Cow` TODO https://doc.rust-lang.org/std/borrow/enum.Cow.html , which
...
if we wrote a one, the solution would not be optimal. Imagine two events for `FinishedGoods`, both having the same `article_id`. They would
...
ArcVal. TODO link to ActyxOS SDK doc.
...
The data model definitions will look as follows:
```
TODO
```
...
interning in the pipelines has usually reduced the memory usage to less than 50% of original usage.


TODO: give links to the examples, add an example for converting the pipeline from previous blogpost, say that it is an exercise for the reader!
TODO: also, for the curious, the link to the Abomonation instance
