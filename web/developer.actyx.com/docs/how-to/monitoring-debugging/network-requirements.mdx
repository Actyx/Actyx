...
Actyx has several [network requirements](../../reference/actyx-reference#networking-requirements). For swarm communication, i.e. communication between nodes, port 4001 is used. You therefore need to verify connections over port 4001 TCP. The below examples assume that you are either executing the command directly from the node, or from another device in the same network:

:::info
In the below examples, we always check port 4001. Which of [these ports](../../reference/actyx-reference#networking-requirements) you need to check depends on the issue you are debugging.
:::
