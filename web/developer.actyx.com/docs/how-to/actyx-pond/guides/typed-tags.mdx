...
where: Tag('chatRoom').withId('my-room').or(Tag('chatRoom').withId('broadcast')) as Where<
  ChatRoomEvent
>

// Events for a specific room, from a specific sender –
// also needs an explicit cast
where: Tag('chatRoom').withId('Melmac').and(Tag('sender').withId('Alf')) as Where<ChatRoomEvent>
```

These events could be emitted as follows:

```typescript
pond.emit(Tag('chatRoom').withId('Melmac').and(Tag('sender').withId('Alf')), {
  type: 'messageAdded',
  message: 'If you love something, let it go.',
})
```
...
pond.emit(tags.chatRoom.withId('Melmac').and(tags.sender.withId('Alf')), {
...
If you want to learn about all the details, check out the [complete guide](../in-depth/tag-type-checking.mdx).
