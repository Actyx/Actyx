...
for emitting events: `pond.emit` and `pond.run` (we'll learn how to use [state effects](state-effects.mdx) later).
...
Actyx however does not treat the `:` in a special way. The Actyx Pond contains some convenience functions based on the convention shown above.
