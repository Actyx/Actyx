...
* Using many fish, each subscribed to a stream with many events, in a single application. We have seen this impacting application startup time noticeably. This scenario benefits from a smaller threshold, as fewer events need to be considered for computation on average in that case.
