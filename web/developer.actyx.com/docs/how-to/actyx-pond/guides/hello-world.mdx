...
"feeds on" little pieces of information – events – produced by connected Actyx nodes. From the
events, it builds its _state_. So when a new event is produced somewhere, the state updates. In your
application, you can _observe_ a fish’s state meaning you get real-time updates!

Let’s start with a very simple fish.

```typescript
import { FishId, Metadata, allEvents, Tag } from '@actyx/pond'

const helloWorldFish = {
  fishId: FishId.of('HelloWorld-Example', 'getting-started', 0), // For caching
  initialState: 'Hello world!',
  onEvent: (_oldState: unknown, event: unknown, _metadata: Metadata) => event,
...
send out a general greeting: _Hello, world!_

Finally, we're providing a `where` clause describing which events this fish is interested in. By
passing `allEvents` we would in fact select all events; which probably results in reading some internal
Actyx events.
By instead requiring any specific tag that is found on 0 events, we preserve the initial state.
...
const main = async () => {
  const pond = await Pond.default({
    ...
    displayName: 'Hello World Tutorial',
    version: '0.0.1',
  })
...
You should see our initial greeting logged to the console – unless your swarm already has some events tagged with `example-tag`,
in which case you will see the latest one!

Hint: Try adding `await pond.emit(Tag('example-tag'), 'Eye eye').toPromise()` to the `main` function.
:::
...
Actyx is all about event streams, so in the next section we explore how events are emitted and thus populate those event streams.
