...
An important aspect of many apps is the involvement of human operators, which requires the presence of a UI that they can see and interact with.

For a UI we need two things: a current state that determines what is shown, and a way to emit events for a fish to
change its state. We have seen how to observe a fish’s state in the [Hello world](hello-world.mdx) section already, and how
to modify it when we discussed [emitting events](state-effects.mdx).
...
export const wireUI = (pond: Pond) => {
  ...
  pond.observe(mkChatRoomFish('my-room'), (state) =>
    ReactDOM.render(<Root msgs={state} sendToRoom={sendToRoom} />, rootEl),
  )
}
```
...
You are welcome to modify the examples and experiment to your liking, and don’t hesitate to drop us a line if you have any questions!
