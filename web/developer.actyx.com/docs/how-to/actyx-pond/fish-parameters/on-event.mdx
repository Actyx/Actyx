...
[Read a concrete example in our tutorial.](../guides/time-travel.mdx)
...
- Returning the input state, unchanged. Although note that you [should not ignore events](../in-depth/do-not-ignore-events.mdx).
...
Actyx uses a specialized mechanism called [Lamport
...
In the rare case that the Lamport timestamps of two events should be identical, other factors are used to
