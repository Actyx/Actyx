...
`where` is for selecting the events that will be passed to a Fish’s [onEvent](./on-event.mdx). Events in
...
You can read more about emitting and selecting events in the [tutorial](../guides/subscriptions.mdx).  
There is also a [detailed guide](../in-depth/tag-type-checking.mdx) covering type-checking best
practices.
