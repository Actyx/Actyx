...
- Full Availability: Applications keep working, even if they become disconnected. I.e. when an
  Actyx node becomes partitioned in the network, all its apps are still completely usable.
...
During a network partition, nodes will necessarily be in disagreement. Once the partition is over,
...
Actyx. Seemingly contradictory information may be created on different nodes. But the
...
An Actyx app can be a huge improvement over confused phone calls, by making contradictions _visible_ and
...
situation. In Actyx apps that is usually a bad idea. In the real world, the damage may already
...
- Do not place too many restrictions on your user interface. Warn about unintended usage! But do
  allow it.
- [Do Not Ignore Events.](do-not-ignore-events.mdx)
...
connected the underlying Actyx currently is. It’s a good idea to make this information available
in the UI via some small indicator ([example](https://github.com/Actyx/industrial-ui/tree/f20671717d0fdd8d59582e6af29edcc892c35adc/src/components/NetworkStatusIcon)).
...
## True Consistency (Strict Serializability)

True consistency can – at the cost of availability – be achieved by using _consensus algorithms_. In
theory, Actyx’ eventually consistent event system is entirely sufficient for building consensus
algorithms on top of it. However, the implementation is not trivial.

A future release of Actyx will ship with a native consensus implementation akin to
