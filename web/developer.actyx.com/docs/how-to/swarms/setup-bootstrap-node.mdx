...
In case you are running ActyxOS on Docker **without** `network=host` , and your bootstrap node is **not** running in the same local network as your other nodes, you need to [configure the `announceAdresses` setting on these nodes](./configure-announced-addresses).
