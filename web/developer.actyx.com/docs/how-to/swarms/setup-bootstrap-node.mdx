...
```text
ax settings set --local com.actyx/swarm/bootstrapNodes [/ip4/192.168.2.225/tcp/4001/p2p/QmQ3iynxmtZUSNF5dvzQQEYhqnB4sqySRej3A2FgiAMBMH] localhost
```
...
In case you are running Actyx on Docker **without** `network=host` , and your bootstrap node is **not** running in the same local network as your other nodes, you need to [configure the `announceAdresses` setting on these nodes](./configure-announced-addresses).
