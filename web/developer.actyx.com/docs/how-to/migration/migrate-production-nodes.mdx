...
Before you start with the migration, please make a copy a v1 data directory onto your PC.
You will need it in the last step for making sure that all nodes were correctly migrated.
...
You will need it later when you check to check if you migrated all nodes and migrate potentially deleted nodes.
...
The migration procedure depends on the operating system of your node–it does not matter in which order you migrate your nodes:

:::tip
We advise that you first make a list of all nodes that should be migrated and update it along the way. Otherwise it will be difficult for you to assess at the end if you actually have deleted nodes that should be migrated, if you just forgot one or more node(s), or if the migration event of a node is not synchronized with the rest of the swarm.
:::
...
You can download the apk here (TODO):

import { DownloadLink } from '../../../src/components/DownloadLink.tsx'

<DownloadLink cta="Download" link="https://actyx.com" />

When starting the app, you are prompted to give Actyx permissions to access the public filesystem so that it can leave a backup of its data there. After accepting the permissions, you should see the following message:

![migration-07](../../../static/images/how-to/migration/migration-07.png)
...
![migration-08](../../../static/images/how-to/migration/migration-08.png)

<h4>4. Clean up your v1 installation</h4>

You can now delete the Actyx v1 apk from your Android device.
...
Therefore, after you have migrated all production nodes, you can now check for any deleted nodes and migrate them.
...
You can download the program for migrating deleted nodes here. TODO

This program needs to be started on the v1 data directory that you copied onto your PC before you started the migration (in this exmaple `actyxos-data-copy`):
...
- Connected v2 peers: Number of migrated v2 nodes that your PC can connect to
...
This is because the v2 node that we are connected to has 2 migration events.
...
You can navigate and select the deleted nodes that should be migrated.
...
In the last step, you get an overview of all nodes that you have migrated from v1 to v2 and a mapping between the source ID and stream ID. You can store this information diretly, but is it also always accessible by querying the migration events as described in the next guide.

:::tip What to do if you find production nodes after finishing the migration
