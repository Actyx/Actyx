...
:::info Usage of tags
When published, each event can be tagged with an arbitrary number of tags. Check out [conceptual guide on tags](../../conceptual/tags.mdx) or our [how-to guides for business logic](../overview.mdx#business-logic) for more info.
:::

:::info Tags with IDs
By convention, events corresponding to a specific instance of a domain entity are tagged with `<entity>:<id>`.
When using [Actyx Pond](../../how-to/business-logic/business-process.mdx), you will have come across [the `Tag.withId()` helper](../../reference/pond/interfaces/Tag#withid).
