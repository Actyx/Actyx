...
In order to be able to run Actyxs on Windows, your device needs to meet the following requirements:
...
You can download the Actyx installer for Windows on our [downloads page](https://downloads.actyx.com).
...
You will now see an Actyx tray icon appear in your taskbar, indicating that Actyx is running.
If you want to stop Actyx, simply right-click on the tray icon and choose _Exit_.
Actyx is now shutting down.
...
Actyx is publicly available for Android in the [Google Play Store](https://play.google.com/store/apps/details?id=com.actyx.android).
...
You can download the binary for Actyx on Linux on our [downloads page](https://downloads.actyx.com).
...
You can download the binary for Actyx on macOS on our [downloads page](https://downloads.actyx.com).
For Actyx to start, please open your terminal, navigate to the directory where you saved the binary and type in the following:
...
Actyx on Docker is published on [DockerHub](https://hub.docker.com/r/actyx/os).
...
```text
docker run --name actyx -it --rm -v actyx-data:/data -p 4001:4001 -p 4458:4458 -p 127.0.0.1:4454:4454 actyx/os
```

<h4>Linux</h4>

```text
docker run --name actyx -it --rm -v actyx-data:/data --network=host actyx/os
```
