...
To do so, we need an entrypoint, we'll call it `run_robot`:
...
export async function run_robot() {
...
Before we proceed, to run the robot, we will need an extra file:

```bash
$ touch src/index.ts
```

And inside it, we import and call our robot entrypoint:

```ts
import { run_robot } from './robot'

run_robot()
```

:::info
This extra file is required because JavaScript will execute the module when importing,
if we called the entrypoint in the `robot.ts` file, everytime we need to import something off of it
we would be launching a new robot!
:::

Finally, to run the robot we recommend adding the following script to your `package.json`:

```json
"scripts": {
  "start-robot": "tsc && node src/index.js"
}
```
...
:::caution
When re-running the tutorial examples, you should stop Actyx and clear its storage.
