...
the client wants us to develop a robot receives the warning and waters the plant.
...
Before starting the tutorial, you will need to have `npm` and `npx` installed
(if they're not installed, you can take a look over their [installation guide](https://github.com/npm/cli#installation)).
:::
...
As stated in the start, we are tasked with developing a robot that waters a plant whenever needed.
The idea is as follows — the plant has a sensor that checks the soil humidity level,
if said level goes bellow a certain threshold, it sends the robot an event requesting water,
...
:::info
To further simplify our example, we will not care for the amount of water we're providing to the plant.
...
"sync" in a way or another so they can cooperate instead of getting in eachothers way,
...
2. This is your Actyx application instance, it will connect to Actyx and get start running your machine.
...
4. Your robot's state machine. Its parameters are:
...
<details>
<summary>You full file should look something like this.</summary>
...
Our mock sensor does not exist yet, so let's write it (I swear its fast).
