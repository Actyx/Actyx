...
This tutorial immediately follows and uses the code of the [previous tutorial](./js-cooperative-task-1-protocol.mdx).
It is recommended to complete that before beginning this tutorial.

:::


[The previous tutorial](./js-cooperative-task-1-protocol.mdx) visits the idea where a swarm (i.e. the robot and the pump) works on one sequential workflow (docking, drawing water, undocking).
...
The previous tutorial illustrates the workflow of a swarm of two (one robot and one water pump) executing a sequential task of docking, drawing water, and undocking. We have an [interaction design](/docs/tutorials/js-cooperative-task-1-protocol#interaction-design) written as a state diagram. 
