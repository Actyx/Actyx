...
- We will get [set up for the tutorial](#setup-for-the-tutorial)
...
In order to get the most out of this tutorial, it is helpful if you have basic knowledge of HTML, CSS, and TypeScript. But since this is a fairly simple example you should be able to follow along even if you are coming from a different programming language.
...
- Download [Actyx for Windows](https://axartifacts.blob.core.windows.net/artifacts/e6ce4c267b1afde174b63745000127f725025750/windows-binaries/windows-x86_64/ActyxOS-Installer.exe)
- Double click and run through the installer
- Start Actyx via the Start Menu; a tray icon will appear

:::info Problems installing?
If you have any problems check out the [Windows installation guide](../os/advanced-guides/actyx-on-windows.md).
:::
...
- Download Actyx for Linux ([64 bit](https://axartifacts.blob.core.windows.net/artifacts/feea217d729d06224838c4d2fcf35ba51c87c64e/linux-binaries/linux-x86_64/actyxos-linux), [ARM 64 bit](https://axartifacts.blob.core.windows.net/artifacts/feea217d729d06224838c4d2fcf35ba51c87c64e/linux-binaries/linux-aarch64/actyxos-linux))
- Open a terminal, navigate to your downloads folder and run the following command `chmod +x actyxos-linux; ./actyxos-linux`

:::info Problems installing?
If you have any problems check out the [Linux installation guides](../os/advanced-guides/actyx-on-linux.md).
:::
...
- Download [Actyx for Mac](https://axartifacts.blob.core.windows.net/artifacts/0010_manual_actyxos-mac-1.1.2/actyxos-mac)
- Open a terminal and run the following command `cd ~/Downloads; chmod +x actyxos-mac; ./actyxos-mac`

:::info Problems installing?
If you have any problems check out the [macOS installation guides](../os/advanced-guides/actyx-on-macos.md).
:::
...
```json file=../samples/chat-app-tutorial/package.json

```

Create another file called `tsconfig.json` with the following content:

```json file=../samples/chat-app-tutorial/tsconfig.json

```

Now create a file called `index.html` and add the following:

```html file=../samples/chat-app-tutorial/index-1.html

```

Finally, create a file named `index.ts` with the following content:

```ts file=../samples/chat-app-tutorial/index-1.ts
...
Actyx enables a completely decentral architecture that allows you to **build apps that always runs**. Your apps always run because they run locally on the device and only interact with the locally running Actyx software.
...
```ts file=../samples/chat-app-tutorial/index.ts#L8:L11
...
```html file=../samples/chat-app-tutorial/index.html
...
```ts file=../samples/chat-app-tutorial/index.ts#L21:L45
...
<DownloadLink
  ...
  link={'https://github.com/Actyx/chat-app-tutorial.git'}
/>
...
- Dive into how it actually works through the Conceptual Guides
