...
If your [ActyxOS Bootstrap Node](actyxos-bootstrap-node.md) is not in the same local network as your ActyxOS nodes; and your ActyxOS nodes are running on Docker without `--network=host`, please read [this paragraph in our troubleshooting section](#actyxos-nodes-not-connecting-to-each-other).
:::
