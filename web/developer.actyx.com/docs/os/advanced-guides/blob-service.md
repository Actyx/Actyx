...
<!--
--------------- Regarding metadata vs. content-addressability (OST) ---------------

Using metadata in a content-addressable storage system begs the question of whether
or not the metadata is part of the content, and thus of the content's address. Off
the top of my head I feel like this shouldn't be the case since metadata is data
about the data (you could say data about the content here). This becomes especially
obvious if you consider storing the Source ID in the metadata. Having the metadata
be a part of the content, would mean that the same files computiationally generated
by two different devices would lead to different hashes. As such, and bear in mind
that this is coming from someone with 0 experience with CAS, I think we can safely
keep metadata outside the hashed content.
