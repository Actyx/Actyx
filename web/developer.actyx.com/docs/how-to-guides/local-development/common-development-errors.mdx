...
1. `4401`: Used for inter-node communication

2. `4243`: Exposes a Websocket endpoint for Actyx Pond (only when running on localhost)

3. `4454`: Exposes the [Event Service](../../reference/actyx-api.mdx) (only when running on localhost)

4. `4457`: Exposes the [Console Service](../../reference/actyx-api.mdx) (only when running on localhost)

5. `8080`: Exposes an [IPFS Gateway](https://docs.ipfs.io/concepts/ipfs-gateway/) (only when running on localhost)
...
- **ActyxOS is not reachable**: This means that ActyxOS is not running on your node.
  Please start ActyxOS described in [this guide](../local-development/installing-actyx.mdx).
