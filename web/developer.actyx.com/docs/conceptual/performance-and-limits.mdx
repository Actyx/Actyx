...
- [Performance](#performance)
  ...
  - [The effect of state snapshots](#the-effect-of-state-snapshots)
...
State snapshots are written when the Fish’s event history is longer than 1,000 events and snapshots are enabled by providing a corresponding (de)serialization configuration.
...
If for example a Fish holds data structures with a volume of 100MB in memory, then storing a snapshot requires serializing this live data structure to JSON (which may separately take more than 100MB) and then sending it to Actyx for safe keeping.
...
The effect of this depends very much on the CPU that is running Actyx: for example 100 sources times 500 Fishes on an Intel Atom processor (rugged Android tablet from around 2015) lead to an internal latency of 3 sec (e.g. between button click and subsequent fish state update).
