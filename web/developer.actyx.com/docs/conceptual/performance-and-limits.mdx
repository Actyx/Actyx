...
Semantics snapshots effectively truncate the event history: everything before the recognized snapshot event is ignored.
...
The effect of this depends very much on the CPU that is running ActyxOS: for example 100 sources times 500 Fishes on an Intel Atom processor (rugged Android tablet from around 2015) lead to an internal latency of 3 sec (e.g. between button click and subsequent fish state update).
...
ActyxOS optimizes event delivery by doing it in chunks, nevertheless in pathological cases the amount of time travel is not linear in the number of such chunks but quadratic.
