...
|----|----|--------|-----------|
|Event|It is recommended to not exceed 4KB per event.|Events have infinite lifespan within the swarm.|Events are replicated on all devices.|
...
For a normal factory use-case, here are two examples based on real data from our customers:
