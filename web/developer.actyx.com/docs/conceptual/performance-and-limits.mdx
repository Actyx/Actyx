...
|----|----|--------|-----------|
|Event|max. 4KB|Events have infinite lifespan unless you reset the swarm.|Events are replicated on all devices.|
|Log|max. 4KB|Logs are cleared automatically when disk space is needed.|Logs are not replicated.|

Until we add these configuration possibilities, the point at which you run out of disk space depends on the size, number, and  compressibility of your events. 
For a normal factory use-case, here is an example based on real data from our customers:

|Type|With machine integrations|Without machine integrations|
|----|-----------------|----------------------------|
|Number of nodes|8|10|
|Operating time|1 year|1 year|
|Number of events|10000000|580000|
