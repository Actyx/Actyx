...
By building and running apps on ActyxOS you are building a decentralized, and thus also a distributed, system. Edge devices provide the computing power and access to the underlying network. Apps run on nodes and communicate by passing messages (see Event Streams). Blobs can be published by any node and accessed from any other node (see Blob Storage).
