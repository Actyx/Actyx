...
- Event expiration should be configured with a safety margin beyond the expected per-fish usage duration:
  waking up a fish whose event history has partially expired will typically result in unusable state.
