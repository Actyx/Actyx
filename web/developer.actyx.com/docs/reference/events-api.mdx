...
This is a reference page for the **Events API**.

The Events API provides local access to the Event Service, allowing you to
...
```bash
export AUTH_TOKEN="$(curl -s localhost:4454/api/v2/authenticate -d'{"appId": "com.example.my-app","displayName": "My Example App","version": "1.0.0"}' -H "Content-Type: application/json" | jq -r '.token')"
```
...
```js
{
  "present": {
    ...
    "f.bEi0RbPe4X3e7bzu5nrRrncTxbcVOXcKtb7vXE0Ok-5": 60
  },
  "toReplicate": {
...
echo '
{
    "lowerBound": {
        ...
        "f.bEi0RbPe4X3e7bzu5nrRrncTxbcVOXcKtb7vXE0Ok-5": -1
    },
    "upperBound": {
