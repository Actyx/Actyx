...
Every `query` must begin with the keyword `FROM`.
Following `FROM`, three different query types can be specified.

- [Tag query](../conceptual/tags.mdx): Arbitrary unicode strings in single quotes, e.g. `FROM 'tag-01'`
- `isLocal`: Returns all events within the specified bounds emitted by the local node backing the Events API
- `allEvents`: Returns all events within the specified bounds known by the node backing the Events API

All three query types can be combined using a combination of the boolean expressions `|` and `&` and parentheses, e.g. `FROM isLocal & ('tag-01' | 'tag-02')`.
...
The `query` field specifies a query that defines how events should be filtered.
Every `query` must begin with the keyword `FROM`.
Following `FROM`, three different query types can be specified.
