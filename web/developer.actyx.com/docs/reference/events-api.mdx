...
Therefore an auth token which is associated with the requesting app needs to be retrieved from the Auth API[./auth-api.mdx].
...
```bash
export AUTH_TOKEN="$(curl -s localhost:4454/api/v2/authenticate -d'{"appId": "com.example.app","displayName": "Example App","version": "1.0"}' -H "Content-Type: application/json" | jq -r '.token')"
```
...
```bash title="Example node_id request"
curl \
    -s -X "GET" \
...
```json title="Example node_id response"
{
  "nodeId": "1g1UOqdpvBB1KHsGWGZiK3Vi8MYGDZZ1oylpOajUk.s"
}
```
...
The `lowerBound` object specifies the lower bound offset for each stream with the numbers being **exclusive**, i.e. a `lowerBound` specification of `34` means the Events API will return events with offsets `> 34`.
...
<!-- TODO: reword (we don't subscribe to streams.) -->

#### Required: Upper bounds for offsets (`upperBound`)

The `upperBound` object specifies the upper bound offset for each stream with the numbers being **inclusive**. i.e. an `upperBound` specification of `34` means the Events API will return events with offsets `<= 34`.
...
The `query` field specifies a tag query that defines which events should be queried.
Every `query` must begin with `FROM` followed by the tags in single quotes.
...
The `offsets` object specifies the lower bound offset for each stream with the numbers being **exclusive**, i.e. a `offsets` specification of `34` means the Events API will return events with offsets `> 34`.

The `offsets` is optional.
If none is set for one, multiple or all subscribed streams, the Event Store will assume a lower bound offset of `-1`, i.e. the beginning.

#### Required: Query (`query`)

The `query` field specifies a tag query that defines how events should be filtered.
Every `query` must begin with `FROM` followed by the tags in single quotes.
...
```bash title="Example subscribe_monotonic request with offsets"
echo '
{
