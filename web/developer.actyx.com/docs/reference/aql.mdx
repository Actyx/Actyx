...
In these cases we very much welcome your questions, comments, and suggestions in the forum!
:::
...
- *FEATURES(timeRange):* `from(<time>)` matches all events whose timestamp is greater or equal to the provided one.
  ...
  Valid formats are `2021-07-20T09:53:07.462Z`, or without millis, seconds, or just the date.
...
- `appId(com.example.my-app)` matches all events from the given app ID.
