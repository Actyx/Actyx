...
NOTE: `FILTER IsDefined(_.count) & _.count > 0` wouldn't work because boolean evaluations are not short circuited.
...
NOTE: Sub-query is a beta feature; the behaviors above MAY be subject to change.

#### Nullifying Error in a Sub-query

Sometimes an error in sub-query — such as possible aggregation missing values — is expected.
In that case, the error can be trapped using the `??` operator and the provided fallback value is returned instead.
...
The AQL has a `last_departure` variable bound to a sub-query
The sub-query uses a specific tag that could yield no events, in which case would fail the following `AGGREGATE`; this is expected by design. 
At the end of the sub-query is `?? NULL`, which provides fallback to the possibly failing sub-query.
