...
Errors can happen during the execution of an AQL query. The following scenarios are the common triggers for those errors:
...
Access to undefined property yields an error.
This commonly occurs because of an incorrect assumption on the shape of an event payload.
...
A common mistake is due to typo that incorrectly refers to the previously declared variable binding.
...
Some aggregation expressions may have a type constraint. For example, `MIN`, `MAX`, `SUM`, and `PRODUCT` only works on boolean and numbers.
...
Another case that cause type mismatch is when an aggregation receives values a sequence of values which the type of does not work with each other in regards with the operation done by the aggregation.
For example, a `MIN` operation accepts boolean and numbers, but it cannot accepts a sequence of a boolean and a number in one pass because a number is incomparable with a boolean.
...
The filter lets zero event pass to the following aggregate command;
The following aggregate command will err.

### Error in subQuery

An error in a subQuery is propagated upward to its parent's

The example below is a subQuery bound to a variable containing an aggregate.
An aggregation may yield an error as shown above; When so, the parent query will yield the subQuery's error.

```text
LET subquery := FROM ... AGGREGATE LAST(_) ... END 
```

### Nullifying Error in subQuery

Sometimes an error in subQuery, such as possible aggregation missing values, is expected.
In that case, the upward propagation of the error could be nullified using the `??` operator, which provides a fallback mechanism.
For an example, take a look into the the AQL below.
...
The AQL has a `last_departure` variable bound to a subQuery
The subQuery uses a specific tag that could yield no events, in which case would fail the following `AGGREGATE`;
And this is expected by design. 
At the end of the subQuery is `?? NULL`, which provides fallback to the possibly failing subQuery.
When the aggregation fails, `last_departure` will be assigned to the value `NULL` instead of raising an error to the parent query.
