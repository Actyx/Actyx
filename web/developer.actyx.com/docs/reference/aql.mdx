...
Optimization by reducing the number of events read, decrypted, decompressed, and decoded.
`.issuedBy` is a string compared with `=` operator, an opportunity to use tags.
Simply publish the event with an accompanying tag `purchase-order:issued-by:[some_id]`; this allows you to write this AQL to replace the tag filter as follows.
...
The new AQL above yields the same result, but the decoding before the `FILTER` line only occurs to the 300 events. 
...
### Discard Most Possible Events Before Sub-Queries

[A Sub-Query](#subquery-sub-queries) in an AQL is executed for each event found by the previous commands.
...
The line "LET revocations ..." invoke the sub-query as many as the number of events found in the `FROM` line.
...
With the above AQL, the `LET revocation ...` line will have a reduced number of events, consequently, a reduced number of sub-queries too.
