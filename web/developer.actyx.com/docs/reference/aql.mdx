...
Our intention is to keep it backwards compatible for as long as we can, so that you can keep running your apps also on future Actyx versions of the 2.x series.
...
Leaving out the `FEATURES()` syntax is the same as enabling no features, meaning that your query can use only released features.
...
- `'mytag'` or `"mytag"` matches all events that carry this tag
  ...
  Quoting it only needed for the used delimiter: if your tag is enclosed in single quotes, single quotes within the tag need to be repeated, e.g. `'o''clock'`.
...
The events originating in your `FROM <tag_expr>` clause may contain more information you need or they may not have the format you desire, e.g. different property names.
