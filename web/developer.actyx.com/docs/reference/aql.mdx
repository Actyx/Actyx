...
Some aggregation expressions may have a type constraint as described [here](#aggregate-aggregating-inputs).
...
Let's see a `MIN` operation for example;
A `MIN` operation accepts boolean and number values, but it cannot compare a boolean with a number;
A sequence of a boolean and a number in one `MIN` aggregation will cause an error.
...
The filter lets zero event pass to the following aggregate command; the following aggregate command will fail.
...
An aggregation may yield an error as shown above; in that case, the parent query will yield the subQuery's error.

```text
LET subquery := FROM ... AGGREGATE LAST(_) ... END 
```

### Nullifying Error in a `subQuery`
...
The subQuery uses a specific tag that could yield no events, in which case would fail the following `AGGREGATE`; this is expected by design. 
