...
pub enum Task {
    ...
    NodeId(PeerId, Sender<ActyxOSResult<NodeId>>),
}
...
pub async fn mk_swarm(key: AxPrivateKey) -> ActyxOSResult<(impl Future<Output = ()> + Send + 'static, Sender<Task>)> {
    ...
    let mut infos = HashMap::<PeerId, (Option<PublicKey>, BTreeSet<String>)>::new();
            ...
            match swarm.poll_next_unpin(cx) {
                Poll::Ready(Some(ev)) => {
                    ...
                    match ev {
                        ...
                        SwarmEvent::Behaviour(OutEvent::Identify(IdentifyEvent::Received { info, peer_id })) => {
                ...
                Poll::Ready(Some(task)) => {
                    ...
                    match task {
                        ...
                        Task::NodeId(peer_id, mut channel) => {
                            let (key, _protos) = match infos.get(&peer_id) {
                                Some(x) => x,
                                None => {
                                    channel
                            ...
                            let node_id = key.map(NodeId::from).ok_or_else(|| {
                                ActyxOSError::new(
...
fn unsupported_proto<T: Debug>(
    infos: Option<&(Option<PublicKey>, BTreeSet<String>)>,
    choices: &[&str],
    tx: &mut Sender<ActyxOSResult<T>>,
) -> bool {
    let (_key, protos) = if let Some(i) = infos {
        i
    } else {
