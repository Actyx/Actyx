...
use crate::{internal::arcval::ArcVal, parse::ParseError};
...
macro_rules! tag {
    ($lit:tt) => {{
        ...
        $crate::scalars::tag::Tag::try_from($lit).unwrap()
    }};
}
...
#[macro_export]
macro_rules! tags {
    () => { $crate::scalars::tag::TagSet::empty() };
    ($($expr:expr),*) => {{
        let mut tags = Vec::new();
        $(
            {
                mod y {
                    ::ax_macros::assert_len! { $expr, 1..,
                        // if it is a string literal, then we know it is not empty
                        pub fn x(z: &str) -> $crate::scalars::tag::Tag {
                            use ::std::convert::TryFrom;
                            $crate::scalars::tag::Tag::try_from(z).unwrap()
                        },
                        // if it is not a string literal, require an infallible conversion
                        pub fn x(z: impl Into<$crate::scalars::tag::Tag>) -> $crate::scalars::tag::Tag {
                            z.into()
                        }
                    }
                }
        ...
        $crate::scalars::tag::TagSet::from(tags)
    }};
    ($($x:tt)*) => {
...
pub struct Tag(#[serde(deserialize_with = "nonempty_string_canonical")] ArcVal<str>);
