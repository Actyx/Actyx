...
mod test {
    ...
    use crate::*;
    use actyx_sdk::{fish_name, semantics, tags, Offset};
    ...
    fn encoded_zstd_test_small() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let cbor_zstd_bytes = std::fs::read("test-data/v1/encoded_zstd_test_small.cbor").unwrap();
    ...
    fn encoded_zstd_test() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let cbor_zstd_bytes: Vec<u8> = std::fs::read("test-data/v1/encoded_zstd_test.cbor")?;
        ...
        assert_eq!(data_cbor_zstd.len(), 50010);
        Ok(())
    }
}
