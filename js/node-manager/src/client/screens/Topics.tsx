...
const LsResultRow = ({
  ...
  if (topic.isActive) {
    return (
      <Row height="7" isChecked={false}>
        {() => jsxCells}
      </Row>
    )
  } else {
    return (
      <Row height="7" isChecked={isChecked} onChecked={toggle} onUnchecked={toggle}>
        {() => jsxCells}
      </Row>
    )
  }
...
function fromTopicLs(response: TopicLsResponseWithAddress): FlatTopic[] {
  return Object.entries(response.topics).map(([name, size]) => ({
...
const LsResults = ({
  ...
  topics: Either<{ nodeAddr: string; error: string }, FlatTopic>[]
  checkedIxs: Set<number>
  toggle: (ix: number) => void
}) => {
  ...
  return (
    <div className="flex-grow mt-6 border-b border-l border-r rounded-md mb-1 text-xs flex flex-col">
      ...
      <div className="flex-grow flex-shrink h-1 overflow-y-scroll overflow-x-hidden">{results}</div>
    </div>
  )
}
...
const Screen: React.FC<{}> = () => {
  ...
  const fetchTopics: () => Promise<
    Either<{ nodeAddr: string; error: string }, TopicLsResponseWithAddress>[]
  > = async () => {
    const topics = await Promise.all(
      nodeManagerAgent.api.getConnectedNodes().map(async (node) => {
        try {
          const res = await nodeManagerAgent.api.getTopicList(node.addr)
  ...
  const deleteTopics = async () => {
    ...
    for (const index of checkedIxs) {
      ...
      if (isRight(topic)) {
        const deleteResult = await nodeManagerAgent.api.deleteTopic(
          topic.right.nodeAddress,
          topic.right.name,
        )
  ...
  return (
    <Layout title="Topic Management">
      <div className="bg-white rounded p-4 min-h-full w-full min-w-full max-w-full overflow-hidden flex flex-col items-stretch h-full">
        ...
        <Button color="red" disabled={checkedIxs.size <= 0} onClick={deleteTopics}>
