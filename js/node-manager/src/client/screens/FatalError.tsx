...
const Screen: React.FC<{ error: FatalError }> = ({ error: { shortMessage, details } }) => {
  const safeDetails = safeErrorToStr(details)
  return (
    <div className="h-full w-full flex justify-center items-center">
      <div className="text-center p-6 px-16">
        <p className="pb-6 text-gray-400 text-sm uppercase font-medium">Fatal error</p>
        <p className="text-red-500">{shortMessage}</p>
        {safeDetails && <p className="text-sm mt-2">{safeDetails}</p>}
        <p className="text-gray-500 mt-8 ">Please exit and restart the Actyx Node Manager.</p>
