...
const mkAnalyticsActions = (client: AmplitudeClient | null): AnalyticsActions => {
  const logEvent = (event: AnalyticsEvent) => {
    if (client === null) {
      if (window !== undefined && (window as any).AX_LOG_ANALYTICS_EVENTS) {
        console.log(`not publishing analytics event (client=null; probably disabled)`)
      }
      return
    }
    if (window !== undefined && (window as any).AX_LOG_ANALYTICS_EVENTS) {
  ...
  return {
    ...
    gotError: (error) => logEvent({ key: 'GotError', shortMessage: sanitizeStr(error) }),
  }
}
...
export const Provider: React.FC<{}> = ({ children }) => {
  ...
  const analyticsDisabled = store.key === StoreStateKey.Loaded && store.data.analytics.disabled
  const analyticsUserId = store.key === StoreStateKey.Loaded && store.data.analytics.userId
