...
const eq = (a: Settings, b: Settings): boolean =>
  ...
  switch (action.key) {
    ...
    case "RemoteUpdated": {
      switch (current.key) {
        ...
        case "DivergedFromInitial": {
          if (
            !eq(current.editor, action.remote) &&
            !eq(current.initial, action.remote)
          ) {
            return {
              key: "DivergedFromBoth",
              editor: current.editor,
    ...
    case "EditorUpdated": {
      switch (current.key) {
        ...
        case "DivergedFromBoth": {
          if (
            eq(current.initial, action.editor) &&
            eq(current.remote, action.editor)
          ) {
            return { key: "NotDiverged", editor: current.editor };
...
export const SettingsEditor: React.FC<Props> = ({
  ...
  const restoreToInitial = () => {
    if (
      state.key === "DivergedFromInitial" ||
      state.key === "DivergedFromBoth"
    ) {
  ...
  const onEditorValueChanged = (val: object) => {
    ...
    setValidationErrors(
      validateAgainstSchema(addr, val, details.settingsSchema)
    );
  };
    ...
    <div className="flex flex-col flex-grow">
      ...
      <div className="flex-grow-0 flex-shrink-0 pt-4 flex flex-row items-center">
        <Button
          ...
          disabled={
            state.key === "SavingToRemote" ||
            state.key === "NotDiverged" ||
            !valid
          }
        ...
        {!(state.key === "SavingToRemote" || state.key === "Error") && (
          <div>
            <span
              className={clsx(
                "text-white text-sm font-medium rounded-full px-4 py-2",
                {
                  "bg-green-500": valid,
                  "bg-red-500": !valid,
                }
      ...
      {state.key === "Error" && (
        <div className="pt-3 text-red-500">Error: {state.reason}</div>
      )}
    </div>
  );
};
