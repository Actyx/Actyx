    ...
    <div className="w-max">
      {nodes.length < 2 ? (
        <p className="text-gray-400">
          Must be connected to at least two nodes to show offset matrix
        ...
        <>
          ...
          {nodes.map((from, ix) => {
            return (
              <div
                key={"om-row-" + from.addr}
                className="flex flew-row text-xs"
              >
                <div
                  className={clsx(
          ...
          <div className="pt-3 text-sm text-gray-400">
            ...
            <p>
              {offsetToString("SourceNotReachable")}: source node not reachable.
            </p>
            <p>
              {offsetToString("TargetNotReachable")}: target node not reachable.
            </p>
            <p>
              {offsetToString("SourceDoesntSupportOffsets")}: source version too
              old.
            </p>
          </div>
        </>
...
const SwarmConnectivity: React.FC<{ nodes: Node[] }> = ({ nodes }) => {
  return (
    <div>
      {nodes.length < 2 ? (
        <p className="text-gray-400">
          Must be connected to at least two nodes to show swarm diagnostics
          (current: {nodes.length}
          ).
        </p>
      ) : (
  ...
  return (
    <div>
      ...
      <p className="py-3 text-gray-400">
        Post this information if you are having issues with the Actyx Node
        Manager.
      </p>
