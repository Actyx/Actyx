    ...
    const mkFavorite = () => {
      ...
      store.actions.updateAndReload({
        ...
        preferences: {
          ...
          favoriteNodeAddrs: store.data.preferences.favoriteNodeAddrs.concat([
            node.addr,
          ]),
        },
      })
        ...
        <p className="text-sm truncate overflow-ellipsis text-gray-300 pr-4">
          <span
            className={clsx("font-medium", {
              "text-red-300":
                node.type === NodeType.Unauthorized ||
      ...
      actions={[
        {
          target: () =>
            remNodes(
              nodes.filter((n) => !isFavorite(store, n)).map((n) => n.addr)
            ),
