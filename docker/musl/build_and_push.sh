...
for target in armv7-unknown-linux-musleabihf x86_64-unknown-linux-musl aarch64-unknown-linux-musl arm-unknown-linux-musleabi; do
  docker buildx build --push --provenance=false --build-arg BUILD_RUST_TOOLCHAIN=$build_rust_toolchain --build-arg TARGET=$target --tag actyx/util:musl-$target-$latest_stable .
done
